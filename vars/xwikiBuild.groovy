...
void call(name = 'Default', body)
{
    ...
    stage("Post Build for ${name}") {
        ...
        // node_modules are excluded because they are deeply nested and cannot contain test recording
        archiveArtifacts artifacts: '**/target/**/*.flv', excludes: '**/node_modules/**', allowEmptyArchive: true
        ...
        // node_modules are excluded because they are deeply nested and cannot contain test screenshots
        archiveArtifacts artifacts: '**/target/**/screenshots/*.png', excludes: '**/node_modules/**',
        ...
        if (config.properties && config.properties.contains('-Dxwiki.test.ui.wcag=true')) {
            ...
            // node_modules are excluded because they are deeply nested and cannot contain wcag reports
            archiveArtifacts artifacts: '**/target/**/wcag-reports/wcag*.txt', excludes: '**/node_modules/**',
                allowEmptyArchive: true
        }
...
private def normalizeTestName(value)
{
    ...
    if (s < 0) { // If no curly braces, look for parentheses
        s = value.indexOf('(')
        e = value.lastIndexOf(')')
    }

    // If no opening bracket found at all, return original
    if (s < 0) return value

    // If closing bracket exists after the start, stitch the prefix and suffix
    // Otherwise, just take the prefix (removes till end of string)
    return (e > s) ? value.substring(0, s) + value.substring(e + 1) : value.substring(0, s)
}
