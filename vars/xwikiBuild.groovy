...
def call(String name = 'Default', body)
{
    ...
    stage("Post Build for ${name}") {
        ...
        def path = "-path '*/target/*' -not \\( -path '*/tmp-jenkins-flv/*' -prune \\)"
        def rsyncCommand = "rsync -R {} tmp-jenkins-flv/"
        sh "find . ${path} -type f -name '*.flv' -exec ${rsyncCommand} \\;"
        ...
        def path = "-path '*/target/*/screenshots' -not \\( -path '*/tmp-jenkins-png/*' -prune \\)"
        rsyncCommand = "rsync -R {} tmp-jenkins-png/"
        sh "find . ${path} -type f -name '*.png' -exec ${rsyncCommand} \\;"
